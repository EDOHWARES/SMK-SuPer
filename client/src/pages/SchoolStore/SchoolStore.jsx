...
const SchoolStore = () => {
  ...
  const addToCart = (product) => {
    setCart((prevCart) => {
      ...
      const existingItem = prevCart.find((item) => item._id === product._id && item.size === product.selectedSize)
  ...
  const updateQuantity = (productId, size, newQuantity) => {
    ...
    setCart((prevCart) =>
      prevCart.map((item) =>
        item._id === productId && item.size === size ? { ...item, quantity: newQuantity } : item,
      ),
    )
  }
          ...
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="mb-8 md:mb-0 md:w-1/2">
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4">
                School Store <span className="text-yellow-400">(Koperasi Sekolah)</span>
              </h1>
              <p className="text-lg md:text-xl mb-6 text-blue-100">
                Your one-stop shop for school uniforms, books, and supplies. Shop online and pick up at school or have
                items delivered.
              </p>
            ...
            <div className="md:w-1/2 flex justify-center">
              <img src="/images/school-store-hero.svg" alt="School Store" className="max-w-full h-auto" />
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-1/4 lg:w-1/5">
            ...
            <div className="bg-white rounded-lg shadow p-4">
              <h2 className="text-xl font-bold text-blue-900 mb-4">User Type</h2>
              <div className="space-y-2">
                <div className="flex items-center p-2 rounded-md hover:bg-gray-100">
                  <input type="radio" id="student" name="userType" className="mr-2" defaultChecked />
                  <label htmlFor="student" className="cursor-pointer w-full">
                    Student
                  </label>
                </div>
                <div className="flex items-center p-2 rounded-md hover:bg-gray-100">
                  <input type="radio" id="parent" name="userType" className="mr-2" />
